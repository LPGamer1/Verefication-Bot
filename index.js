...
async function iniciarBanco() {
    if (process.env.DATABASE_URL) {
        ...
        try {
            // Cria a tabela se não existir (Correção para o erro de tabela não encontrada)
            await pool.query(`
...
app.get('/callback', async (req, res) => {
    ...
    try {
        const tokenResponse = await axios.post('https://discord.com/api/oauth2/token', new URLSearchParams({
            ...
            code, grant_type: 'authorization_code', redirect_uri: process.env.REDIRECT_URI, scope: 'identify guilds.join',
    ...
    await client.application.commands.set([
        ...
        { 
            ...
            options: [
                { 
                    name: 'quantidade', 
                    description: 'Quantas pessoas enviar (máximo do estoque)', // CORRIGIDO
                    type: 4, 
                    required: true 
                },
                { 
